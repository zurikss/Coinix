...
struct CoinModel: Identifiable, Codable {
    ...
    /*
     init(from decoder: Decoder) throws {
         ...
         // Handle the case where "current_price" is not a Double or is missing
         if let currentPriceString = try? container.decode(String.self, forKey: .currentPrice) {
             if let currentPrice = Double(currentPriceString) {
                 self.currentPrice = currentPrice
             } else {
                 self.currentPrice = nil // Unable to convert to Double
             }
         } else {
